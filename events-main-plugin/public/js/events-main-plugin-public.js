( function($){
    ...
    function HandleBrowseClick(fileinput) {
        let browse = $('#' + fileinput).parents('.field-group').find('.form-control').attr('id');
        $('#' + browse).trigger('click');
    }

    function Handlechange(fileinput) {
        let textinput = $('#' + fileinput).parents('.file-upload-wrap').find('.filename').attr('id');
        let textinputDom = document.getElementById(textinput);
        let fileinputDom = document.getElementById(fileinput);
        let allowedFileType = $('#' + fileinput).attr('allowed-file-type');
