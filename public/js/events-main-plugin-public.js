...
jQuery(document).ready(function () {
    ...
    $(document).on('click', '.button-wrap .register', function (e) {
        ...
        var errorMessage = '';
        var error = false;
        var radioBtnValidation='';
        var checkboxBtnValidation='';
        var selectBtnValidation='';
        var notValidateMessage = '';
        var validateMessage = '';
        var emailValidateMessage = '';
        var codeValidateMessage = '';
        var urlValidateMessage = '';
        var emailExistMessage = '';
            ...
            if ($(this).hasClass('required')) {
                if ($(this).hasClass('google-captcha')) {
                    ...
                    if (0 === $(this).find('input[type="radio"]:checked').length) {
                        ...
                        $(this).addClass('invalid');
                        $(this).removeClass('valid');
                        var label = $(this).find('.field-label').contents().eq(0).text();
                        if( '' !== label ){
                    ...
                    var textAreaNotValidateMessage = '';
                    if ('ar' === lang) {
                    ...
                    var inputFieldNotValidationMessage = '';
                    var label = $(this).find('.field-label').contents().eq(0).text();
                    if( '' !== label ){
                        label = label.toLowerCase();
                    }
        ...
        if (0 === $('.invalid').length) {
            ...
            var email = $('.field-wrap input[type="email"]').val();
            jQuery.ajax({
                url: formObj.ajaxurl,
                type: 'POST',
                dataType: 'json',
                ...
                success: function (result) {
                    if (result.status) {
                        var fieldWrap = $('.field-wrap input[type="email"]').parents('.field-wrap');
...
function Handlechange(fileinput) {
    ...
    if (-1 < $.inArray('doc', exts)) {
        exts.push('docx');
    }
    if (-1 < $.inArray('pptx', exts)) {
        exts.push('ppt');
    }
    if (-1 < $.inArray('xlsx', exts)) {
        exts.push('xls');
    }
    ...
    if (fileinputDom) {
        if (0 < fileinputDom.files.length) {
            ...
            if (0 === uploadfileError) {
                ...
                $('#' + fileinput).parents('.field-container').find('.field-label').append('<span class="validate">' + validateMessage + '</span>');
                textinputDom.value = files;
            } else {
                $('#' + fileinput).parents('.field-wrap').removeClass('valid').addClass('invalid');
                $('#' + fileinput).parents('.field-container').find('.field-label').append('<span class="not-validate">' + fileUploadMessage + '</span>');
                textinputDom.value = '';
            }
        } else {
            if ($('#' + fileinput).parents('.field-wrap').hasClass('required')) {
                $('#' + fileinput).parents('.field-wrap').removeClass('valid').addClass('invalid');
                $('#' + fileinput).parents('.field-container').find('.field-label').append('<span class="not-validate">' + fileFieldNotValidationMessage + '</span>');
                textinputDom.value = '';
...
console.log('diffmainTranslations', formObj.diffmainTranslations);
console.log('diffmainTest', formObj.diffmainTest)